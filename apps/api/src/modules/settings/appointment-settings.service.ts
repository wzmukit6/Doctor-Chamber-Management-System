...
/** Chamber-level appointment & token configuration (spec §7, §8, §34 "Appointment Settings"). */
@Injectable()
export class AppointmentSettingsService {
  ...
  async get(chamberId: string): Promise<AppointmentSettings & { version: number }> {
    const row = await this.prisma.setting.findUnique({
      where: { scope_scopeId_key: { scope: 'CHAMBER', scopeId: chamberId, key: KEY } },
    });
    if (!row) return { ...DEFAULT_APPOINTMENT_SETTINGS, version: 0 };
    // Merge with defaults so newly added settings get sensible values.
    const parsed = appointmentSettingsSchema.safeParse({ ...DEFAULT_APPOINTMENT_SETTINGS, ...(row.value as object) });
    return { ...(parsed.success ? parsed.data : DEFAULT_APPOINTMENT_SETTINGS), version: row.version };
  }

  async update(actor: Actor, chamberId: string, input: UpdateAppointmentSettingsInput) {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      await this.audit.record(
        actor,
        { action: 'settings.appointments_updated', resourceType: 'settings', resourceId: `${chamberId}:${KEY}`, oldValue: old, newValue: value, chamberId },
        tx,
      );
    });
