...
export class PasswordService {
  ...
  /** Checks the platform password policy (Settings → Security). */
  async assertStrong(password: string) {
    const issues = passwordIssues(password, await this.security.current());
    if (issues.length) {
      throw new AppError(
